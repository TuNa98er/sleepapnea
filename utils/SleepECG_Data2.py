...
def load_data():
    ...
    for i in range(len(o_train)):
        ...
        # if np.any(np.isnan(s_ampl_interp_signal))==False:
        rri_interp_signal = splev(tm, splrep(rri_tm, scaler(rri_signal), k=3), ext=1) 
        r_ampl_interp_signal = splev(tm, splrep(r_ampl_tm, scaler(r_ampl_siganl), k=3), ext=1)
        
        x_train.append([rri_interp_signal, r_ampl_interp_signal,r_ampl_interp_signal])
        # print(x_train)
    ...
    for i in range(len(o_test)):
        # (rri_tm, rri_signal), (r_ampl_tm, r_ampl_siganl) ,(s_ampl_tm, s_ampl_siganl) = o_test[i]
        ...
        x_test.append([rri_interp_signal, r_ampl_interp_signal,r_ampl_interp_signal])
        # print(x_test)
    # print()   
