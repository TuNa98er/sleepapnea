...
base_dir = "/home/tuan/tuna/sleepapnea/SA-2.0.0"
base_save = "/home/tuan/tuna/sleepapnea"
...
def Teager_power_function(Signal):
    ...
    # discrete Teager The formula of the energy operator =S（n）*S（n）-S(n+1)*S*(n-1)
...
def worker(name, labels):
    ...
    for j in range(len(labels)):
        ...
        # Find R peaks
        signal=Teager_power_function(signal)
        ...
        if np.all(np.logical_and(hr >= hr_min, hr <= hr_max)):
            ...
            X.append([(rri_tm, rri_signal), (r_ampl_tm, r_ampl_siganl)])
